...
def name_to_one_hot_num(dev_name, num_labels):
    """
    Converts a device name to a one-hot encoded label.
    :param dev_name: name of the device to convert to one-hot
    :return the one-hot encoded device name
    ...
    output = np.zeros((1, num_labels))  # 16 possible outputs
    i = int(dev_name)
    if i >= 0 and i < num_labels:
        output[0, i] = 1
    else:
        print("No matching device, you should interrupt this and solve the problem.")
    return output
...
def gather_everything_file_list(file_list, label_list, num_labels, num_vals=20006400, verbose=False):
    num_files = len(file_list)
    labels = np.zeros((num_files, num_labels), dtype=np.int16)
    big_data = np.zeros((num_files, num_vals, 2), dtype=np.float32)
    for index,file in enumerate(file_list):
        print(file)
        big_data[index, :, :] = parse_data('', file, num_vals)
        print('labels')
        labels[index, :] = name_to_one_hot_num(label_list[index], num_labels)
        print('done')
        
    return big_data, labels
        
        
def gather_everything(path, num_vals=20006400, num_files=32, pattern=r'.*(sigmf-data)', tx_id_pattern = r'(X310_)(.*)(_.*)(_.*)(.sigmf-data)', verbose=False):
    """
...
def gather_everything2(path, num_vals=20006400, num_files=32, pattern=r'.*(sigmf-data)', tx_id_pattern = r'(X310_)(.*)(_.*)(_.*)(.sigmf-data)', verbose=False):
    """
    Given the path to the high level data, usually the KRI folder level, will return all of the data from
    a specific distance.
    :param path: string pointing to the distance folder you want to retreive the data from
    ...
    for file in file_list:
        if re.search(pattern, file) and index < num_files:
            ...
            data_files.append(file)
            big_data[index, :, :] = parse_data(file, path, num_vals)
            labels[index, :] = name_to_one_hot2(re.search(tx_id_pattern, file).group(2))
            index += 1
...
def restructure_data(in_data, in_labels, out_sample_size=256, complex_val=False, verbose=False):
    """
...
def tfr_write(in_data, devices, path,  distance=2, compressed=True, tag=None, verbose=False, skip_cast=False):
    """
    ...
    # create file name
    if tag is not None:
        file_name = str(sets)+'_' + str(samples) + '_' + str(distance) + 'ft_data_' + str(tag) + '.tfrecord'
    else:
